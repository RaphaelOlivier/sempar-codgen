...
def evaluate(model, dataset, verbose=True):
    ...
    for example in dataset.examples:
        ...
        assert len(predict_rules) > 0 and len(gfirst_rules) > 0

        exact_match = sorted(gfirst_rules, key=lambda x: x.__repr__()) == sorted(predict_rules, key=lambda x: x.__repr__())
        if exact_match:
            exact_match_ratio += 1

        # p = len(predict_rules.intersection(gfirst_rules)) / len(predict_rules)
        # r = len(predict_rules.intersection(gfirst_rules)) / len(gfirst_rules)
...
def evaluate_decode_results(dataset, decode_results, verbose=True):
    ...
    for eid in range(dataset.count):
        ...
        if config.data_type == 'django':
            ...
            for literal, place_hfirster in example.meta_data['str_map'].iteritems():
                pred_code_for_bleu = pred_code_for_bleu.replace('\'' + place_hfirster + '\'', literal)
                # ref_code_for_bleu = ref_code_for_bleu.replace('\'' + place_hfirster + '\'', literal)
...
def decode_and_evaluate_ifttt(model, test_data):
    raw_ids = [int(i.strip()) for i in open(config.ifttt_test_split)]  # 'data/ifff.test_data.gfirst.id'
...
def decode_and_evaluate_ifttt_by_split(model, test_data):
    for split in ['ifff.test_data.omit_non_english.id', 'ifff.test_data.omit_unintelligible.id', 'ifff.test_data.gfirst.id']:
        raw_ids = [int(i.strip()) for i in open(os.path.join(config.ifttt_test_split), split)]  # 'data/ifff.test_data.gfirst.id'
        eids = [i for i, e in enumerate(test_data.examples) if e.raw_id in raw_ids]
