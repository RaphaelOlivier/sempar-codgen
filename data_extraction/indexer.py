...
def export_id(my_map, output_file):
	...
	for token, index in my_map.iteritems():
		writer.write("%r"%str(token) + "\t" + str(index) + "\n")
	writer.close()
...
def set_to_id(terminal_node_set, output_file):
	...
	for element in terminal_node_set:
		writer.write("%r"%str(element) + "\t" + str(index) + "\n")
		index += 1
	writer.close()
...
def run_hs():
	...
	export_id(train_annot, "../data/indexer/annot_vocab_hs.tsv")
	train_terminal = train_data.terminal_vocab.token_id_map
	export_id(train_terminal, "../data/indexer/terminal_vocab_hs.tsv")
	train_grammar = train_data.grammar.rule_to_id
	export_id(train_grammar, "../data/indexer/rules_hs.tsv")
	train_nodes = train_data.grammar.node_type_to_id
	export_id(train_nodes, "../data/indexer/nodes_hs.tsv")
	train_terminal = dev_data.grammar.terminal_nodes
	set_to_id(train_terminal, "../data/indexer/frontier_nodes_hs.tsv")
	print("done HS")
...
def run_django():
	...
	train_annot = train_data.annot_vocab.token_id_map
	export_id(train_annot, "../data/indexer/annot_vocab_django.tsv")
	train_terminal = train_data.terminal_vocab.token_id_map
	export_id(train_terminal, "../data/indexer/terminal_vocab_django.tsv")
	train_grammar = train_data.grammar.rule_to_id
	export_id(train_grammar, "../data/indexer/rules_django.tsv")
	train_nodes = train_data.grammar.node_type_to_id
	export_id(train_nodes, "../data/indexer/nodes_django.tsv")
	train_terminal = dev_data.grammar.terminal_nodes
	set_to_id(train_terminal, "../data/indexer/frontier_nodes_django.tsv")
	print("done Django")
